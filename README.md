# WriteSpace Backend API

> A robust, scalable backend for a modern blogging platform.

## 🚀 Overview

WriteSpace is a RESTful API built with **Node.js** and **TypeScript**, designed to power a professional blogging platform. It supports advanced features like role-based authentication, rich content management, social interactions (likes, comments, shares), and asynchronous notifications.

## 🛠️ Tech Stack

- **Runtime**: Node.js (v18+)
- **Framework**: Express.js
- **Language**: TypeScript
- **Database**: MongoDB (Mongoose)
- **Caching & Queues**: Redis, BullMQ
- **Authentication**: Passport.js, JWT
- **Documentation**: Swagger (OpenAPI), TypeDoc
- **Testing**: Jest, Supertest

## 📂 Project Structure

```
writespace-backend/
├── .github/                   # CI/CD Workflows (GitHub Actions)
│   └── workflows/
│       ├── test.yml           # Runs tests on Pull Request
│       └── deploy.yml         # Deploys to AWS/DigitalOcean
│
├── .vscode/                   # VS Code settings (optional but good for teams)
│   └── settings.json
│
├── dist/                      # Compiled JavaScript files (Generated by TypeScript)
│
├── docs/                      # Documentation
│   └── swagger.yaml           # OpenAPI Specification (optional if using JSDoc)
│
├── logs/                      # Local logs (error.log, combined.log)
│   └── .keep
│
├── src/
│   ├── config/                # Global Configurations
│   │   ├── db.ts              # MongoDB Connection Logic
│   │   ├── redis.ts           # Redis Client Setup
│   │   ├── env.ts             # Environment Variable Validation (Joi/Zod)
│   │   ├── swagger.ts         # Swagger JSDoc Setup
│   │   └── logger.ts          # Winston/Pino Logger Setup
│   │
│   ├── modules/               # VERTICAL SLICES (Feature Based)
│   │   ├── auth/              # Authentication Feature
│   │   │   ├── dtos/          # Data Transfer Objects (Validation)
│   │   │   │   ├── login.dto.ts
│   │   │   │   ├── register.dto.ts
│   │   │   │   └── verify-otp.dto.ts
│   │   │   ├── interfaces/    # TypeScript Interfaces
│   │   │   │   └── auth.interface.ts
│   │   │   ├── auth.controller.ts  # HTTP Request Handler
│   │   │   ├── auth.service.ts     # Business Logic
│   │   │   ├── auth.routes.ts      # API Routes definition
│   │   │   └── auth.utils.ts       # Helper (e.g., JWT Generator)
│   │   │
│   │   ├── users/             # User Management
│   │   │   ├── dtos/
│   │   │   │   └── update-profile.dto.ts
│   │   │   ├── interfaces/
│   │   │   │   └── user.interface.ts
│   │   │   ├── users.controller.ts
│   │   │   ├── users.service.ts
│   │   │   ├── users.routes.ts
│   │   │   └── users.model.ts      # Mongoose Schema
│   │   │
│   │   ├── posts/             # Blog Posts
│   │   │   ├── dtos/
│   │   │   │   ├── create-post.dto.ts
│   │   │   │   └── update-post.dto.ts
│   │   │   ├── interfaces/
│   │   │   │   └── post.interface.ts
│   │   │   ├── posts.controller.ts
│   │   │   ├── posts.service.ts
│   │   │   ├── posts.routes.ts
│   │   │   └── posts.model.ts
│   │   │
│   │   ├── interactions/      # Likes, Shares, Comments
│   │   │   ├── dtos/
│   │   │   │   └── add-comment.dto.ts
│   │   │   ├── interfaces/
│   │   │   │   ├── like.interface.ts
│   │   │   │   └── comment.interface.ts
│   │   │   ├── interactions.controller.ts
│   │   │   ├── interactions.service.ts
│   │   │   ├── interactions.routes.ts
│   │   │   ├── likes.model.ts
│   │   │   └── comments.model.ts
│   │   │
│   │   └── notifications/     # Email & SMS Service
│   │       ├── interfaces/
│   │       │   └── email.interface.ts
│   │       ├── email.service.ts    # SendGrid/AWS SES Logic
│   │       ├── sms.service.ts      # Twilio Logic
│   │       └── templates/          # HTML Email Templates
│   │           ├── welcome.html
│   │           └── otp.html
│   │
│   ├── shared/                # Code shared across modules
│   │   ├── constants/         # Magic Strings/Numbers
│   │   │   └── http-codes.ts
│   │   │
│   │   ├── infra/             # External Services Wrappers
│   │   │   └── mailer.ts
│   │   │
│   │   ├── middlewares/       # Express Middlewares
│   │   │   ├── auth.middleware.ts     # JWT Verification
│   │   │   ├── error.middleware.ts    # Global Error Handler
│   │   │   ├── validate.middleware.ts # Request Body Validator
│   │   │   ├── rate-limit.middleware.ts # Prevent DDoS
│   │   │   └── upload.middleware.ts   # Multer (Image Uploads)
│   │   │
│   │   ├── queues/            # Message Queue Producers/Consumers
│   │   │   ├── email.queue.ts       # Defines the Queue
│   │   │   ├── email.worker.ts      # Processes the Queue
│   │   │   └── interaction.queue.ts # Handles "Likes"
│   │   │
│   │   ├── utils/             # Helper Functions
│   │   │   ├── api-response.ts      # Standard Success Response
│   │   │   ├── app-error.ts         # Standard Error Class
│   │   │   └── catch-async.ts       # Try-Catch Wrapper
│   │   │
│   │   └── types/             # Global TypeScript Types
│   │       └── express.d.ts     # Extending Express Request (req.user)
│   │
│   ├── app.ts                 # Express Application Setup (Middleware)
│   └── server.ts              # Entry Point (Server Listener)
│
├── tests/                     # Unit & Integration Tests
│   ├── unit/
│   │   ├── auth.service.test.ts
│   │   └── posts.service.test.ts
│   └── integration/
│       ├── auth.routes.test.ts
│       └── posts.routes.test.ts
│
├── .dockerignore              # Files to ignore in Docker
├── .env.example               # Template for environment variables
├── .eslintrc.json             # Linting Rules
├── .gitignore                 # Git Ignore
├── .prettierrc                # Formatting Rules
├── docker-compose.yml         # Run Redis/Mongo/App locally
├── Dockerfile                 # For building the production image
├── jest.config.ts             # Testing Configuration
├── nodemon.json               # Auto-restart config
├── package.json               # Dependencies
├── README.md                  # Project Documentation
└── tsconfig.json              # TypeScript Configuration
```

## ⚡ Getting Started

### Prerequisites

- Node.js & npm
- MongoDB
- Redis

### Installation

1. **Clone the repository**
   ```bash
   git clone https://github.com/your-username/writespace-backend.git
   cd writespace-backend
   ```

2. **Install dependencies**
   ```bash
   npm install
   ```

3. **Configure Environment**
   Create a `.env` file in the root directory (copy `.env.example`).
   ```env
   PORT=5000
   MONGO_URI=mongodb://localhost:27017/WriteSpace
   REDIS_URL=redis://localhost:6379
   JWT_SECRET=your_jwt_secret
   CLIENT_URL=http://localhost:3000
   ```

4. **Run Development Server**
   ```bash
   npm run dev
   ```

## 🧪 Testing

Run the test suite:

```bash
# Run all tests
npm test

# Run tests with coverage
npm run test:cov
```

## 📖 Documentation

- **API Docs**: Run `npm run docs` to generate API reference in `docs/api`.
- **User Guide**: See `docs/user-guide/getting-started.md`.

## 📜 License

This project is licensed under the MIT License - see the [LICENSE](LICENSE) file for details.
